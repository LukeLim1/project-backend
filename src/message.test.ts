...
describe('messageShareV1', () => {
  let user1: Response, user1Body: { token: string[]; authUserId: number; },
    ...
    dm1Body: { identifier: number; }, send1: Response, send1Body: any,
  ...
  beforeEach(() => {
    ...
    expect(dm1Body).toMatchObject({ identifier: expect.any(Number) });
    ...
    // send message to dm
    dmMessage1 = dmSend(user1Body.token[0], dm1Body.identifier, 'DM 1 message 1 ahaha');
  ...
  test('Testing successful messageShare 200 status code', () => {
    dmSend(user1Body.token[0], dm1Body.identifier, 'DM 1 message 2 beheheh');
    // new message in dm that is concatenation of first dm and new dm
    const res = shareMessage(dmMessage1Body.messageId, 'new message ahahah', -1, dm1Body.identifier);
  ...
  test('message length exceeds 1000 chars 400 error', () => {
    ...
    expect(shareMessage(dmMessage1Body.messageId, messageToGive, -1, dm1Body.identifier)).toHaveProperty('statusCode', 400);
  });
  test('channel id and dmid are invalid 400 error', () => {
    expect(shareMessage(dmMessage1Body.messageId, 'any message', -1, dm1Body.identifier + 999)).toHaveProperty('statusCode', 400);
  ...
  test('neither channelid nor dmid are -1 400 error', () => {
    expect(shareMessage(dmMessage1Body.messageId, 'any message', chan1Body.channelId, dm1Body.identifier)).toHaveProperty('statusCode', 400);
  });
