...
describe('userRemove tests using Jest', () => {
    test('Test successful userRemove', () => {
        ...
        const res = userRemove(newUser.authUserId);
    ...
    test('uId doesn\'t refer to valid user', () => {
        const newUser = JSON.parse(String(createBasicAccount().getBody()));
        const res = userRemove(newUser.authUserId + 5);
        const bodyObj = JSON.parse(String(res.getBody()));
        expect(res.statusCode).toBe(OK);
        // expect 400 error
    });
...
describe('userPermissionChange tests using Jest', () => {
    test('Test successful userPermissionChange', () => {
        const newUser = JSON.parse(String(createBasicAccount().getBody()));
        const res = userPermissionChange(newUser.authUserId, 1);
        const bodyObj = JSON.parse(String(res.getBody()));
        expect(res.statusCode).toBe(OK);
        expect(bodyObj).toMatchObject({});
    });
    ...
    test('uId does not refer to a valid user', () => {
        const newUser = JSON.parse(String(createBasicAccount().getBody()));
        const res = userPermissionChange(newUser.authUserId + 5, 1);
        const bodyObj = JSON.parse(String(res.getBody()));
        expect(res.statusCode).toBe(OK);
        // expect 400 error
    });
    ...
    test('permission ID invalid', () => {
        const newUser = JSON.parse(String(createBasicAccount().getBody()));
        const res = userPermissionChange(newUser.authUserId, 10000);
        const bodyObj = JSON.parse(String(res.getBody()));
        expect(res.statusCode).toBe(OK);
        // expect 400 error
    });
